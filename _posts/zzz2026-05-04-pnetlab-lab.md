...
>
> FortiGate `port2` (WAN) kết nối vào Cloud0, đặt IP tĩnh `10.10.200.40/24`, gateway pfSense `10.10.200.1`.
...
# Cấu hình port2 (WAN) — IP tĩnh, gateway pfSense
config system interface
    edit port2
        set mode static
...
# Nếu thiếu route này: Ubuntu ping FortiGate → FortiGate reply ra port2 (WAN)
...
**Debug từng hop khi không ra được Internet:**

Nếu `ping 8.8.8.8` không thông, kiểm tra lần lượt từng bước:

```bash
# Bước 1 — Kiểm tra IP và route trên Ubuntu
ip -br a
route -n
# Kỳ vọng: eth1 có 192.168.20.10/24, default via 192.168.20.1

# Bước 2 — Ping Router (hop kế tiếp)
ping -c3 192.168.20.1
# Nếu FAIL → kiểm tra lại Switch VLAN hoặc cable link

# Bước 3 — Ping FortiGate port1 từ Ubuntu
ping -c3 192.168.30.2
# Nếu FAIL → Router chưa có route hoặc FortiGate port1 chưa up

# Bước 4 — Ping FortiGate WAN từ Ubuntu
ping -c3 10.10.200.40
# Nếu FAIL → FortiGate chưa apply cấu hình port2

# Bước 5 — Ping pfSense gateway
ping -c3 10.10.200.1
# Nếu FAIL → FortiGate WAN chưa có default route

# Bước 6 — Kiểm tra Internet bằng TCP/UDP (pfSense chặn ICMP, chỉ cho phép port 80/443/53/123)
# ĐỪNG dùng ping 8.8.8.8 → sẽ luôn fail vì pfSense block ICMP outbound
...
> **Lưu ý pfSense:** pfSense trong lab này chỉ cho phép outbound các port:
> - **80/443** — HTTP/HTTPS
> - **53** — DNS
> - **123** — NTP
>
> ICMP (ping) ra Internet bị chặn → `ping 8.8.8.8` luôn fail dù Internet thực sự thông. Dùng `curl` hoặc `apt update` để verify connectivity.
